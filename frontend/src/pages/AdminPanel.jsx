  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {error && (
          <div className="rounded-2xl bg-red-50 border border-red-200 p-4 text-sm text-red-900 shadow-sm mb-6">
            {error}
          </div>
        )}
        ...
        <section className="grid gap-6 xl:grid-cols-4 mb-8">
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            ...
            <p className="mt-3 text-3xl font-semibold text-gray-900">{stats?.user_stats?.total_users ?? '-'}</p>
          </div>
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <p className="text-sm text-gray-500">Agents actifs</p>
            <p className="mt-3 text-3xl font-semibold text-gray-900">{stats?.user_stats?.agents ?? '-'}</p>
          </div>
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <p className="text-sm text-gray-500">Super Agents</p>
            <p className="mt-3 text-3xl font-semibold text-gray-900">{stats?.user_stats?.super_agents ?? '-'}</p>
          </div>
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <p className="text-sm text-gray-500">Demandes en attente</p>
            <p className="mt-3 text-3xl font-semibold text-gray-900">{stats?.user_stats?.pending_super_agents ?? '-'}</p>
          </div>
        </section>

        <section className="grid gap-6 xl:grid-cols-3 mb-8">
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Stocks actuels</h2>
            <p className="mt-2 text-sm text-gray-600">Ajustez les quantités en cas d’erreur d’enregistrement.</p>
            <div className="mt-6 space-y-4">
              {stocks.map((stock) => (
                <div key={stock.network} className="grid grid-cols-1 gap-3 rounded-2xl border border-gray-200 bg-gray-50 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                  <div>
                    <p className="font-medium text-gray-900">{stock.network}</p>
                    <p className="text-sm text-gray-600">Quantité actuelle : {stock.quantity}</p>
                  </div>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Ventes du jour</h2>
            <p className="mt-2 text-sm text-gray-600">Synthèse des ventes et des écarts.</p>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Rôles et droits</h2>
            <p className="mt-2 text-sm text-gray-600">Attribuez des rôles et approuvez les super agents en attente.</p>
            <div className="mt-6 space-y-4">
              {stats && (
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200 mb-8">
          <h2 className="text-lg font-semibold text-gray-900">Taux de Change</h2>
          <p className="mt-2 text-sm text-gray-600">Définissez le taux de change USD vers FC.</p>
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200 mb-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600">Modifiez les rôles et approuvez les super agents.</p>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {users.map((userRecord) => (
                  <tr key={userRecord.id}>
                    <td className="px-4 py-4 text-gray-700">{userRecord.name}</td>
                    <td className="px-4 py-4 text-gray-700">{userRecord.email}</td>
                    ...
                    <td className="px-4 py-4 text-gray-700">{userRecord.status}</td>
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Statistiques réseau</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {stats?.sales_stats?.network_sales && Object.entries(stats.sales_stats.network_sales).map(([network, quantity]) => (
              <div key={network} className="rounded-2xl bg-gray-50 p-4">
                <p className="text-sm text-gray-500">{network}</p>
                <p className="mt-3 text-2xl font-semibold text-gray-900">{quantity}</p>
              </div>
            ))}
          </div>
        </section>
